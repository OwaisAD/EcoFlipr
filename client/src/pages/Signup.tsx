...
const initialState = {first_name:"", last_name:"",address:"",phone_number:"",email:"",password:"", confirm_password: ""}
...
const navigate = useNavigate();
 ...
 const handleSignUp: React.MouseEventHandler<HTMLButtonElement> = (event) => {
  ...
  if (!signUpCredentials.first_name || signUpCredentials.first_name.length < 2 || signUpCredentials.first_name.length > 50 ){
    toast.error("First name must be between 2 and 50 characters", {position: "top-right"})
    return;
  }
  if (!signUpCredentials.last_name ||signUpCredentials.last_name.length < 2 ||signUpCredentials.last_name.length > 50 ){
      ...
      <div className="bg-white flex flex-col justify-center py-[35px] px-[70px] w-full md:max-w-full lg:max-w-[640px]">
        ...
        >Email</label>
          ...
          <input type="password" placeholder="Confirm password" id="confirm_password" className="rounded-[0.2rem]"/>
            ...
            <input type="checkbox" />
            <p>I agree to these <span className="text-blue-700">Terms and Conditions</span></p>
          </div>

<div className="flex flex-col items-center justify-center gap-2 mt-2">
  <div>
     <button type="submit" className="bg-blue-700 text-white px-6 py-2 rounded-full" onClick={handleSignUp}>Sign up</button>
  </div>
